...
interface ClaimTransferClientProps {
  initialToken: string;
}

export function ClaimTransferClient({ initialToken }: ClaimTransferClientProps) {
  const searchParams = useSearchParams();
  const tokenFromUrl = searchParams.get("token")?.trim() ?? "";
  const token = tokenFromUrl || initialToken.trim();
  ...
  useEffect(() => {
    if (!token) {
      setPreview(null);
      setLookupError("This transfer link is missing a token. Open the full link from your invitation email.");
      setLoading(false);
      return;
    }
  ...
  if (!token || !preview?.valid || preview.expired) {
    return (
      <div className="air-card w-full max-w-lg air-card-pad text-center">
